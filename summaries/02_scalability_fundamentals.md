...
- **Vertical (Scale-up):** Adding more CPU/RAM to a single machine.
  - _Pros:_ No app changes needed. Good for stateful/legacy systems (e.g., primary relational DB).
  - _Cons:_ Superlinear cost, hard ceiling, single point of failure.
- **Horizontal (Scale-out):** Adding more machines.
  - _Pros:_ Linear cost, no architectural ceiling, handles failures gracefully (N+1 redundancy).
  - _Cons:_ Requires stateless architecture, adds coordination overhead (load balancers, network).
...
- _Rule of thumb:_ Scale-out should be fast/aggressive; scale-in should be slow/conservative to avoid flapping.
...
- **Architecture:**
  ...
  - API Gateway (Stateless, L7 LB using _least-connections_).
